...
namespace ProductBundles.UnitTests
{
    ...
    public class MongoProductBundleInstanceStorageTests
    {
        ...
        [TestMethod]
        public async Task GetAllAsync_WithNoInstances_ShouldReturnEmptyCollection()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
                TestDatabaseName,
                TestCollectionName + "_empty_" + Guid.NewGuid().ToString("N")[..8],
                _logger);
            ...
            // Assert
            Assert.IsNotNull(instances, "GetAllAsync should return a collection");
            Assert.AreEqual(0, instances.Count(), "Collection should be empty");
        }

        [TestMethod]
        public async Task GetAllAsync_WithMultipleInstances_ShouldReturnAllInstances()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
                TestDatabaseName,
                TestCollectionName + "_multi_" + Guid.NewGuid().ToString("N")[..8],
                _logger);
            
            var instance1 = new ProductBundleInstance(
                "getall-test-1-" + Guid.NewGuid(),
                "test-plugin-1",
                "1.0.0");
            instance1.Properties["prop1"] = "value1";
            
            var instance2 = new ProductBundleInstance(
                "getall-test-2-" + Guid.NewGuid(),
                "test-plugin-2",
                "2.0.0");
            instance2.Properties["prop2"] = "value2";
            
            var instance3 = new ProductBundleInstance(
                "getall-test-3-" + Guid.NewGuid(),
                "test-plugin-1", // Same plugin as instance1
                "1.1.0");
            instance3.Properties["prop3"] = "value3";
            ...
            try
            {
                ...
                // Assert
                Assert.IsNotNull(allInstances, "GetAllAsync should return a collection");
                var instanceList = allInstances.ToList();
                Assert.AreEqual(3, instanceList.Count, "Should return all 3 instances");
                
                // Verify all instances are present
                var ids = instanceList.Select(i => i.Id).ToHashSet();
                Assert.IsTrue(ids.Contains(instance1.Id), "Should contain instance1");
                Assert.IsTrue(ids.Contains(instance2.Id), "Should contain instance2");
                Assert.IsTrue(ids.Contains(instance3.Id), "Should contain instance3");
                
                // Verify properties are preserved
                var retrievedInstance1 = instanceList.First(i => i.Id == instance1.Id);
                Assert.AreEqual("value1", retrievedInstance1.Properties["prop1"]);
                
                var retrievedInstance2 = instanceList.First(i => i.Id == instance2.Id);
                Assert.AreEqual("value2", retrievedInstance2.Properties["prop2"]);
                
                var retrievedInstance3 = instanceList.First(i => i.Id == instance3.Id);
                Assert.AreEqual("value3", retrievedInstance3.Properties["prop3"]);
            }
            finally
            {
                // Cleanup: try to delete all test instances
                try
                {
                    await storage.DeleteAsync(instance1.Id);
                    await storage.DeleteAsync(instance2.Id);
                    await storage.DeleteAsync(instance3.Id);
                }
                catch
                {
                    // Ignore cleanup errors
        ...
        [TestMethod]
        public async Task GetAllAsync_WithSingleInstance_ShouldReturnSingleInstance()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
                TestDatabaseName,
                TestCollectionName + "_single_" + Guid.NewGuid().ToString("N")[..8],
                _logger);
            
            var instance = new ProductBundleInstance(
                "getall-single-" + Guid.NewGuid(),
                "test-plugin",
                "1.0.0");
            instance.Properties["testProp"] = "testValue";
            ...
            try
            {
                ...
                Assert.IsNotNull(allInstances, "GetAllAsync should return a collection");
                var instanceList = allInstances.ToList();
                Assert.AreEqual(1, instanceList.Count, "Should return exactly 1 instance");
                
                var retrievedInstance = instanceList[0];
                Assert.AreEqual(instance.Id, retrievedInstance.Id);
                Assert.AreEqual(instance.ProductBundleId, retrievedInstance.ProductBundleId);
                Assert.AreEqual(instance.ProductBundleVersion, retrievedInstance.ProductBundleVersion);
                Assert.AreEqual("testValue", retrievedInstance.Properties["testProp"]);
            }
            finally
            {
                // Cleanup: try to delete the test instance
        ...
        #region GetByProductBundleIdAsync Tests

        [TestMethod]
        public async Task GetByProductBundleIdAsync_WithNonExistentProductBundleId_ShouldReturnEmptyCollection()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
                TestDatabaseName,
                TestCollectionName + "_nonexistent_" + Guid.NewGuid().ToString("N")[..8],
                _logger);
            
            var nonExistentId = "non-existent-plugin-" + Guid.NewGuid();

            // Act
            var instances = await storage.GetByProductBundleIdAsync(nonExistentId);

            // Assert
            Assert.IsNotNull(instances, "GetByProductBundleIdAsync should return a collection");
            Assert.AreEqual(0, instances.Count(), "Collection should be empty for non-existent ProductBundle ID");
        }

        [TestMethod]
        public async Task GetByProductBundleIdAsync_WithExistingProductBundleId_ShouldReturnMatchingInstances()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
                TestDatabaseName,
                TestCollectionName + "_existing_" + Guid.NewGuid().ToString("N")[..8],
                _logger);
            
            var targetProductBundleId = "target-plugin-" + Guid.NewGuid();
            var otherProductBundleId = "other-plugin-" + Guid.NewGuid();
            
            var instance1 = new ProductBundleInstance(
                "getbyplugin-test-1-" + Guid.NewGuid(),
                targetProductBundleId,
                "1.0.0");
            instance1.Properties["prop1"] = "value1";
            
            var instance2 = new ProductBundleInstance(
                "getbyplugin-test-2-" + Guid.NewGuid(),
                targetProductBundleId,
                "1.1.0");
            instance2.Properties["prop2"] = "value2";
            
            var instance3 = new ProductBundleInstance(
                "getbyplugin-test-3-" + Guid.NewGuid(),
                otherProductBundleId, // Different plugin
                "2.0.0");
            instance3.Properties["prop3"] = "value3";
            ...
            try
            {
                ...
                // Act
                var matchingInstances = await storage.GetByProductBundleIdAsync(targetProductBundleId);

                // Assert
                Assert.IsNotNull(matchingInstances, "GetByProductBundleIdAsync should return a collection");
                var instanceList = matchingInstances.ToList();
                Assert.AreEqual(2, instanceList.Count, "Should return exactly 2 instances for target ProductBundle ID");
                
                // Verify only matching instances are returned
                var ids = instanceList.Select(i => i.Id).ToHashSet();
                Assert.IsTrue(ids.Contains(instance1.Id), "Should contain instance1");
                Assert.IsTrue(ids.Contains(instance2.Id), "Should contain instance2");
                Assert.IsFalse(ids.Contains(instance3.Id), "Should NOT contain instance3 (different ProductBundle ID)");
                
                // Verify all returned instances have the correct ProductBundle ID
                Assert.IsTrue(instanceList.All(i => i.ProductBundleId == targetProductBundleId), "All returned instances should have the target ProductBundle ID");
        ...
        [TestMethod]
        public async Task GetByProductBundleIdAsync_WithSingleMatchingInstance_ShouldReturnSingleInstance()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
                TestDatabaseName,
                TestCollectionName + "_single_match_" + Guid.NewGuid().ToString("N")[..8],
                _logger);
            
            var targetProductBundleId = "single-target-plugin-" + Guid.NewGuid();
            var otherProductBundleId = "other-plugin-" + Guid.NewGuid();
            
            var targetInstance = new ProductBundleInstance(
                "getbyplugin-single-" + Guid.NewGuid(),
                targetProductBundleId,
                "1.0.0");
            targetInstance.Properties["targetProp"] = "targetValue";
            
            var otherInstance = new ProductBundleInstance(
                "getbyplugin-other-" + Guid.NewGuid(),
                otherProductBundleId,
                "2.0.0");
            otherInstance.Properties["otherProp"] = "otherValue";

            try
            {
                // Create both instances
                await storage.CreateAsync(targetInstance);
                await storage.CreateAsync(otherInstance);
                ...
                Assert.IsNotNull(matchingInstances, "GetByProductBundleIdAsync should return a collection");
                var instanceList = matchingInstances.ToList();
                Assert.AreEqual(1, instanceList.Count, "Should return exactly 1 instance for target ProductBundle ID");
                
                var retrievedInstance = instanceList[0];
                Assert.AreEqual(targetInstance.Id, retrievedInstance.Id);
                Assert.AreEqual(targetProductBundleId, retrievedInstance.ProductBundleId);
                Assert.AreEqual("1.0.0", retrievedInstance.ProductBundleVersion);
                Assert.AreEqual("targetValue", retrievedInstance.Properties["targetProp"]);
            }
            finally
            {
                // Cleanup: try to delete test instances
                try
                {
                    await storage.DeleteAsync(targetInstance.Id);
                    await storage.DeleteAsync(otherInstance.Id);
                }
                catch
                {
                    // Ignore cleanup errors
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentException))]
        public async Task GetByProductBundleIdAsync_WithNullProductBundleId_ShouldThrowArgumentException()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
            ...
            // Act
            await storage.GetByProductBundleIdAsync(null!);
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentException))]
        public async Task GetByProductBundleIdAsync_WithEmptyProductBundleId_ShouldThrowArgumentException()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentException))]
        public async Task GetByProductBundleIdAsync_WithWhitespaceProductBundleId_ShouldThrowArgumentException()
        {
            // Arrange
            var storage = new MongoProductBundleInstanceStorage(
                TestConnectionString,
